...
const detectWebsiteType = (message: string): string => {
    const lowerMessage = message.toLowerCase();
    if (lowerMessage.includes('portfolio') || lowerMessage.includes('artist') || lowerMessage.includes('designer')) return 'Portfolio';
    if (lowerMessage.includes('ecommerce') || lowerMessage.includes('shop') || lowerMessage.includes('store')) return 'E-commerce';
    if (lowerMessage.includes('blog') || lowerMessage.includes('news') || lowerMessage.includes('article')) return 'Blog';
    if (lowerMessage.includes('restaurant') || lowerMessage.includes('food') || lowerMessage.includes('cafe')) return 'Restaurant';
    if (lowerMessage.includes('saas') || lowerMessage.includes('app') || lowerMessage.includes('software')) return 'SaaS';
    if (lowerMessage.includes('agency') || lowerMessage.includes('business') || lowerMessage.includes('company')) return 'Business';
    return 'General';
};

const detectWebsiteSize = (message: string): 'small' | 'medium' | 'large' => {
    const lowerMessage = message.toLowerCase();
...
const selectTheme = (message: string) => {
    const lowerMessage = message.toLowerCase();
...
const generateBrandName = (message: string): string => {
    const lowerMessage = message.toLowerCase();
    ...
    const names = brandNames[websiteType as keyof typeof brandNames] || brandNames['General'];
    return names[Math.floor(Math.random() * names.length)];
};

const generateNavigation = (websiteType: string): string => {
    const navItems = {
    ...
    return navItems[websiteType as keyof typeof navItems] || navItems['General'];
};

const generateHeroTitle = (message: string, websiteType: string): string => {
    const lowerMessage = message.toLowerCase();
    ...
    return titles[websiteType as keyof typeof titles] || titles['General'];
};

const generateHeroDescription = (message: string, websiteType: string): string => {
    const lowerMessage = message.toLowerCase();
    ...
    return descriptions[websiteType as keyof typeof descriptions] || descriptions['General'];
};

const generateFeatures = (websiteType: string, theme: any): string => {
    const features = {
        'Portfolio': `
    ...
    return features[websiteType as keyof typeof features] || features['General'];
};

// Advanced fallback website generator that creates sophisticated websites
const generateFallbackWebsite = (messages: ChatMessage[]): string => {
